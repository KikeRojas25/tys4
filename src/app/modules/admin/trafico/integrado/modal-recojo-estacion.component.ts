...
@Component({
    template: `
        <div class="w-full p-4">
            <h3 class="text-lg font-semibold mb-4">Detalle de Recojo - {{ titulo }}</h3>

            <div class="flex flex-wrap gap-2 mb-4 items-end">
                <div class="flex-1 min-w-[200px]">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Buscar por Número o Distrito:</label>
                    <input
                        pInputText
                        type="text"
                        [(ngModel)]="busqueda"
                        (input)="filtrar()"
                        placeholder="Ej: OR-001234 o LIMA"
                        class="w-full">
                </div>
                <div>
                    <button
...
export class ModalRecojoEstacionComponent implements OnInit {
    ...
    constructor(
        ...
        this.titulo = config.data?.titulo || 'Detalle de Recojo';
    }

    ngOnInit() {
        this.cols = [
            { field: 'numcp',          header: 'Número OR'      },
        ...
        this.traficoService.GetDetalleRecojoXEstacion(idProvincia).subscribe({
            next: (data) => {
                this.registros = data;
    ...
    exportarExcel(): void {
        ...
        import('xlsx').then((xlsx: any) => {
            ...
            import('file-saver').then((FileSaver) => {
                ...
                FileSaver.default.saveAs(blob, `DetalleRecojo_${new Date().getTime()}.xlsx`);
            });
        });
    }
