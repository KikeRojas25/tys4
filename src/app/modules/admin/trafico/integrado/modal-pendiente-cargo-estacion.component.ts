            ...
            <p-table
                ...
                <ng-template pTemplate="body" let-row>
                    <tr>
                        ...
                        <td class="text-center">{{ row.idmanifiesto ?? '-' }}</td>
                    </tr>
                </ng-template>
...
export class ModalPendienteCargoEstacionComponent implements OnInit {
    ...
    constructor(
        ...
        this.titulo = config.data?.titulo || 'Detalle Recabar Cargos';
    }

    ngOnInit() {
        const idProvincia = this.config.data?.idProvincia;
        const tipo = this.config.data?.tipo;
        const fuente = tipo === 'enviocargo'
            ? this.traficoService.GetDetalleEnvioCargo(idProvincia)
            : this.traficoService.GetDetallePendienteCargo(idProvincia);
    ...
    exportarExcel(): void {
        ...
        import('xlsx').then((xlsx: any) => {
            ...
            import('file-saver').then((FileSaver) => {
                ...
                FileSaver.default.saveAs(blob, `RecabarCargos_${new Date().getTime()}.xlsx`);
            });
        });
    }
