...
@Component({
    template: `
        <div class="w-full p-4">
            <h3 class="text-lg font-semibold mb-4">Detalle de Recepción - {{ titulo }}</h3>

            <div class="flex flex-wrap gap-2 mb-4 items-end">
                <div class="flex-1 min-w-[200px]">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Buscar por Número, Manifiesto o Distrito:</label>
                    <input
                        pInputText
                        type="text"
                        [(ngModel)]="busqueda"
...
export class ModalRecepcionEstacionComponent implements OnInit {
    ...
    constructor(
        ...
        this.titulo = config.data?.titulo || 'Detalle de Recepción';
    }

    ngOnInit() {
        this.cols = [
            { field: 'numcp',         header: 'Número OT'    },
            { field: 'nummanifiesto', header: 'Manifiesto'   },
            { field: 'numhojaruta',   header: 'Hoja de Ruta' },
            { field: 'fecharegistro', header: 'F. Registro'  },
            { field: 'origen',        header: 'Origen'       },
            { field: 'peso',          header: 'Peso'         },
            { field: 'bulto',         header: 'Bultos'       },
            { field: 'pesovol',       header: 'Peso Vol.'    },
            { field: 'estado',        header: 'Estado'       },
        ];

        const idProvincia = this.config.data?.idProvincia;

        this.traficoService.GetDetalleOrdenesxEstacionRecepcion(idProvincia).subscribe({
            next: (data) => {
                this.registros = data;
    ...
    exportarExcel(): void {
        ...
        import('xlsx').then((xlsx: any) => {
            ...
            import('file-saver').then((FileSaver) => {
                ...
                FileSaver.default.saveAs(blob, `DetalleRecepcion_${new Date().getTime()}.xlsx`);
            });
        });
    }
