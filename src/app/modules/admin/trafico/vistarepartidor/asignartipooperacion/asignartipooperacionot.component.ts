...
@Component({
  selector: 'app-asignartipooperacionot',
  templateUrl: './asignartipooperacionot.component.html',
  styleUrls: ['./asignartipooperacionot.component.css'],
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class AsignartipooperacionOtComponent implements OnInit {
  ...
  guardar() {
      ...
      // Puede recibir un solo ID o múltiples IDs separados por coma
      // Si es masivo, recibimos una cadena de IDs separados por coma
      // Si es individual, recibimos un solo número
      const data = this.config.data;
      
      if (data.esMasivo && typeof data.idOrdenTrabajo === 'string') {
        // Procesamiento masivo: múltiples IDs separados por coma
        this.model.ids = data.idOrdenTrabajo;
      } else {
        // Procesamiento individual: un solo ID
        this.model.ids = data.idOrdenTrabajo;
      }
    ...
    if(this.model.idtipooperacion=== 123) {
      ...
      // Usar asignarTipoOperacion (funciona tanto para individual como masivo)
      // El servicio procesará todos los IDs en el campo ids
