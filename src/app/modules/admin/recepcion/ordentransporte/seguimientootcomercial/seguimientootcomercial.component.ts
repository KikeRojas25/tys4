...
@Component({
  selector: 'app-seguimientootcomercial',
  templateUrl: './seguimientootcomercial.component.html',
  styleUrls: ['./seguimientootcomercial.component.css'],
  standalone: true,
  imports: [ 
    FormsModule,
    CommonModule,
...
export class SeguimientootcomercialComponent implements OnInit {
  ...
  opcionesAgrupacion: SelectItem[] = [
    { value: 'provinciaDisplay', label: 'Provincia' },
  ...
  ngOnInit() {
    ...
    // Leer idcliente desde query params (ej. desde integrado comercial al hacer clic en cliente)
    const qp = this.route.snapshot.queryParams;
    const idcliente = qp['idcliente'];
    if (idcliente != null && idcliente !== '' && !isNaN(Number(idcliente))) {
      this.model.idcliente = Number(idcliente);
    }
    ...
    [
        ...
        {header: 'OT / CLIENTE', field: 'numcp'  ,  width: '180px' },
        {header: 'F. RECOJO', field: 'fecharegistro' , width: '100px'  },
        {header: 'F. DESPACHO', field: 'fecharegistro' , width: '100px'  },
        {header: 'F. ENTREGA', field: 'fecharegistro' , width: '100px'  },
        {header: 'ESTACIÓN', field: 'razonsocial'  ,  width: '140px'  },
        {header: 'ESTADO', field: 'estado'  , width: '100px'   },
        {header: 'SUB-ESTADO', field: 'subestado'  , width: '120px'   },
        {header: 'INCID. ENTREGA', field: 'tipoentrega'  , width: '110px'   },
    ...
    this.ordenTransporteService.getClientes(this.user.idscliente).subscribe(resp => {
        ...
        // No sobrescribir idcliente si viene de query params (ej. desde integrado comercial)
        if (this.model.idcliente == null || this.model.idcliente === undefined) {
...
editarConfirm(id) {
  ...
  extraerProvincia(provincia: string): string {
    if (!provincia) return 'Sin destino';
    const partes = (provincia || '').split(' - ');
    return partes.length >= 2 ? partes[1].trim() : provincia;
  }

  prepararDatosParaGrilla(): void {
    const fuente = this.ordenesFiltradas.length > 0 ? this.ordenesFiltradas : this.ordenes;
    const mapeados = fuente.map(o => ({
      ...o,
      provinciaDisplay: this.extraerProvincia(o.provincia)
    }));
    const campo = this.groupByField || 'provinciaDisplay';
    const ordenados = [...mapeados].sort((a, b) =>
      (a[campo] || '').toString().localeCompare((b[campo] || '').toString())
    );

    if (this.groupByField) {
      const gruposUnicos = new Set(ordenados.map(o => (o[campo] || '').toString()));
      if (this.expandedGroups.size === 0) {
        gruposUnicos.forEach(g => this.expandedGroups.add(g));
      }
      const resultado: any[] = [];
      let i = 0;
      while (i < ordenados.length) {
        const row = ordenados[i];
        const groupVal = (row[campo] || '').toString();
        const expanded = this.expandedGroups.has(groupVal);
        if (expanded) {
          resultado.push(row);
          i++;
        } else {
          const groupRows = ordenados.filter(o => (o[campo] || '').toString() === groupVal);
          resultado.push({ ...groupRows[0], _groupCollapsed: true, _groupCount: groupRows.length });
          i += groupRows.length;
        }
      }
      this.ordenesParaGrilla = resultado;
    } else {
      this.ordenesParaGrilla = ordenados;
    }
  }
