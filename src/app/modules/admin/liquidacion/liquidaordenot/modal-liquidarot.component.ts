...
@Component({
  selector: 'app-modal-liquidarot',
  standalone: true,
  imports: [CommonModule, FormsModule, DropdownModule, InputTextareaModule, ButtonModule, ToastModule],
  template: `
    <div class="p-2">
      <p *ngIf="ot" class="text-sm text-gray-600 mb-3">OT: <strong>{{ ot }}</strong></p>
...
export class ModalLiquidarOtComponent {
  ...
  liquidar(): void {
    ...
    this.liquidacionService
      .liquidarOT(this.id, this.model.maestroincidenciaid, this.model.observacion ?? '')
      .subscribe({
        next: () => this.ref.close(true),
        error: (err) => {
          console.error('Error al liquidar OT:', err);
          this.messageService.add({
            severity: 'error',
            summary: 'Liquidación',
            detail: err?.error?.message ?? 'No se pudo liquidar la orden.',
          });
        },
