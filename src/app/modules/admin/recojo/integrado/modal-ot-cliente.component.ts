...
import { ComercialService, OrdenTrabajoDetallePorClienteEstadoResult } from '../../comercial/comercial.service';
...
export class ModalOtClienteComponent implements OnInit {
  ordenes: OrdenTrabajoDetallePorClienteEstadoResult[] = [];
  ordenesFiltradas: OrdenTrabajoDetallePorClienteEstadoResult[] = [];
  cols: any[] = [];
  titulo = '';
  busquedaNumero = '';
  ...
  ngOnInit(): void {
    ...
    const idcliente = this.config.data?.idcliente;
    const estados = this.config.data?.estados;

    if (idcliente && estados !== undefined && estados !== null) {
      this.comercialService.getOrdenesDetallePorClienteEstado(idcliente, estados).subscribe({
        next: (data) => {
          this.ordenes = data ?? [];
